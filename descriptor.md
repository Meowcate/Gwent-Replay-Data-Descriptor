...
# Gwent Replay Data Descriptor v1.3.1
...
Gwent action | GRDD action code | Note
--- | --- | ---
Pass | ``pass`` | Note : after a player passed, all of his potential turns after this must have at least one step, even if it's an empty step (with nothing but ``"step": 1``). Some actions can happens during a player's turn even if he passed.
Forfeit | ``forfeit`` | When the current player surrender. The other player wins.
Disconnect | ``disconnect`` | When the current player had a connection problem. The other player wins.
End game | ``endgame`` | When both player passed. Endgame should always be put in a step where current `turn.player` is the losing player, as the last step of his turn (generally after a `pass`) or in a new turn right after the other's player turn. The other player wins.
Tie game | ``tie`` | When the game ends and both players have the same score.
...
> **Note :** The replay doesn't calculate the scores and crowns to know who is the winner, the file must always have a final step specify who won the game (if the replay is a game running until the end).
